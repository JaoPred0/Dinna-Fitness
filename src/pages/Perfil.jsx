...
import {
    User, Mail, Calendar, LogIn, Shield, Edit, Loader2,
    MailCheckIcon,
    MailWarningIcon,
    CheckCircle
} from "lucide-react";
...
const Perfil = () => {
    ...
    useEffect(() => {
        ...
        // Só dispara se o e-mail não estiver verificado
        if (!user.emailVerified) {
            const interval = setInterval(() => {
                alert("⚠️ Lembre-se de verificar seu e-mail para ativar sua conta!");
                // Aqui você pode substituir alert por toast ou notificação mais elegante
    ...
    const ProfileAvatar = () => (
        <div className="relative flex justify-center">
            ...
            <motion.div
                className={`absolute -bottom-0.5 -right-0.5 sm:-bottom-1 sm:-right-1 md:-bottom-2 md:-right-2 p-1 sm:p-1.5 md:p-2 rounded-full border-3 sm:border-4 border-white shadow-lg
        ${user.emailVerified ? 'bg-green-500' : 'bg-yellow-500'}`}
            ...
            >
                {user.emailVerified ? (
                    <MailCheckIcon className="w-2.5 h-2.5 sm:w-3 sm:h-3 md:w-5 md:h-5 text-white" />
                ) : (
                    <MailWarningIcon className="w-2.5 h-2.5 sm:w-3 sm:h-3 md:w-5 md:h-5 text-white" />
                )}
            </motion.div>
        </div>
    );

    const getAccessLevel = (userEmail) => {
        const adminEmails = import.meta.env.VITE_ADMIN_EMAILS
            ? import.meta.env.VITE_ADMIN_EMAILS.split(",")
            : [];
        const programmerEmails = import.meta.env.VITE_PROGRAMMER_EMAILS
            ? import.meta.env.VITE_PROGRAMMER_EMAILS.split(",")
            : [];

        if (programmerEmails.includes(userEmail)) return "Programador";
    ...
    const getAccountAge = (creationDateStr) => {
        const creationDate = new Date(creationDateStr);
        if (isNaN(creationDate)) return "";
        ...
        if (diffYears > 0) return `${diffYears} ano(s)`;
        if (diffMonths > 0) return `${diffMonths} mês(es)`;
                                ...
                                <div className="flex items-center gap-1 mt-1 sm:mt-2 flex-wrap">
                                    {user.emailVerified ? (
                                        <motion.span
                                            ...
                                            animate={{ scale: 1 }}
                                        >
                                            <CheckCircle className="w-3 h-3" /> {/* Ícone verde de verificado */}
                                            E-mail Verificado
                                        </motion.span>
                                    ) : (
                                        <div className="flex gap-1.5 sm:gap-2 items-center flex-wrap mt-1">
                                            <span className="text-yellow-600 text-xs">E-mail Não Verificado</span>
                                            <button
                            ...
                            <div className="bg-gray-50 p-2 sm:p-3 md:p-4 rounded-lg sm:rounded-xl md:col-span-2">
                                <div className="flex items-center gap-1.5 sm:gap-2 mb-1.5 sm:mb-2">
                                    {accessLevel === "Admin" ? (
                                        ...
                                        <>
                                            <User className="w-3.5 h-3.5 sm:w-4 sm:h-4 md:w-5 md:h-5 text-purple-600 flex-shrink-0" />
                                            <span className="font-semibold text-purple-700 text-xs sm:text-sm md:text-base">Nível de Acesso</span>
                                        </>
                                    ) : (
                                        <>
