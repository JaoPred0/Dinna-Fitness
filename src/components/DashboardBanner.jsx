...
import { Image, Save, X, Edit3, PlusCircle, Trash2, RefreshCw, ArrowLeft } from "lucide-react";
import { db } from "../config/firebase";
import { doc, setDoc, getDoc } from "firebase/firestore";
import Slider from "react-slick";
import { useNavigate } from "react-router-dom"; // 👈 Importante para o botão voltar

const DashboardBanner = () => {
  const navigate = useNavigate(); // 👈 Hook do React Router

  useEffect(() => {
    document.title = "Dashboard - Banners";
  }, []);
    ...
    <div className="min-h-screen p-6">
      ...
      <div className="mb-8">
        {banners.length > 0 ? (
          ...
          <div className="text-gray-500 text-center p-8 border rounded-xl bg-white">
                ...
                {tempBanners.map((banner, i) => (
                  <div key={i} className="space-y-2 border p-3 rounded-lg">
                    <input
                      ...
                      value={banner.image}
                      onChange={(e) =>
                        handleUpdateBanner(i, "image", e.target.value)
                      }
                    ...
                    <input
                      ...
                      value={banner.link}
                      onChange={(e) =>
                        handleUpdateBanner(i, "link", e.target.value)
                      }
